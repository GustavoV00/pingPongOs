...
void ppos_init() {
  ...
  if (flag == 0) {
    ...
    timer.it_interval.tv_usec =
        1000;                     // disparos subsequentes, em micro-segundos
...
// Indica se volta para a main ou se vai para o dispatcher que é o gerenciador
// e tarefas.
void task_yield() {
// Caso seja diferente, após a tarefa atual ser executada, a task_yield vai ser
// hamada E coloca a tarefa que acabou de ser executada no final da fila.
// Chamando novamente o task_switch para continuar o dispatcher
#ifdef DEBUG
  printf("Quando a tarefa atual, for a main novamente, significa que chegou no "
         "fim.\n");
#endif
...
void task_exit(int exit_code) {
  ...
  printf("Task %d exit: execution time %d ms, processor time %d ms, %d "
         "activations\n",
         task_id(), TarefaAtual->duracaoDaTarefa,
         TarefaAtual->tempoNoProcessador, TarefaAtual->ativacoes);
#ifdef DEBUG
...
task_t *scheduler() {
  ...
  task_t *FilaAux =
      (task_t *)FilaTarefas; // FilaxAux é o que vai andar pela Fila
...
unsigned int systime() { return ticks; }
...
void task_ticks(int sinal) {
  ...
  if (sinal == 14 && TarefaAtual->tarefaUsuario == 1 && block == 1) {
    quantum -= 1;
    if (quantum ==
        0) { // Quando chega a zero, indica que seu tempo no processador acabou
      quantum = 20;
      task_yield();
    }
  }
...
int mqueue_send(mqueue_t *queue, void *msg) {
  ...
  if (queue->ultimo == queue->max_msg - 1)
    queue->ultimo = -1;
  queue->ultimo += 1;
  memcpy(queue->buffer + (queue->msg_size * queue->ultimo), msg,
         queue->msg_size);
  queue->quantidade += 1;
...
int mqueue_recv(mqueue_t *queue, void *msg) {
  ...
  memcpy(msg, queue->buffer + (queue->msg_size * queue->primeiro),
         queue->msg_size);
  queue->primeiro += 1;
  queue->quantidade -= 1;
  if (queue->primeiro == queue->max_msg)
    queue->primeiro = 0;
