...
void ppos_init()
{
    ...
    if (flag == 0)
    {
        ...
        // Flag de sinal do p13
        actionDisk.sa_handler = diskSinal;
        sigemptyset(&action.sa_mask);
...
void task_ticks(int sinal)
{
    ...
    if (sinal == 14 && TarefaAtual->tarefaUsuario == 1 && block == 1)
    {
        quantum -= 1;
        if (quantum == 0)
        { // Quando chega a zero, indica que seu tempo no processador acabou
            quantum = 20;
            task_yield();
        }
    }
